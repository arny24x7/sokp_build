...
# Copyright (C) 2012/2013 SOKP Project
#
...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from SOKP Github (http://github.com/SOKP)." % device

repositories = []

page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/SOKP/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/sokp_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'SOKP/%s already exists' % (repo_name)
            else:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "sokp", "name": repo_name, "revision": "sokp-443" })
    ...
    f = open('.repo/local_manifests/sokp_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for SOKP/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: SOKP/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "sokp", "name": "SOKP/%s" % repo_name, "revision": "sokp-443" })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/sokp.dependencies'
    syncable_repos = []
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if repo_name.startswith("device_") and repo_name.endswith("_" + device):
            ...
            add_to_manifest([{'repository':repo_name,'target_path':repo_path,'branch':'sokp-443'}])
...
print "Repository for %s not found in the SOKP Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/sokp_manifest.xml" % device
