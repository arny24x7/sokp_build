...
sed -i 's/^ro\.build\.version\.release=.*/ro.build.version.release=4.4-sokp/g' ${ANDROID_HOME}/platforms/android-${SDK_VER}-${CUSTOM_NAME}/build.prop
...
sed -i 's/Pkg.Desc=/Pkg.Desc=SOKP /' ${ANDROID_HOME}/platforms/android-${SDK_VER}-${CUSTOM_NAME}/source.properties
