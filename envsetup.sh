...
- pstest:   cherry pick a patch from the SOKP gerrit instance.
- pspush:   push commit to SOKP gerrit instance.
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^sokp_") ; then
       SOKP_PRODUCT=$(echo -n $1 | sed -e 's/^sokp_//g')
    else
       SOKP_PRODUCT=
    fi
      export SOKP_PRODUCT
...
{
    target=$1
    SOKP_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/sokp/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the SOKP model name
            lunch sokp_$target-userdebug
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=`sed -n -e'/ro\.sokp\.version/s/^.*=//p' $OUT/system/build.prop`
...
function pstest() {
    ...
    gerrit=gerrit.sokp.co
    project=`git config --get remote.sokp.projectname`
    patch="$1"
...
function pspush() {
    if [ -z "$1" ] || [ "$1" = '--help' ]; then
        ...
        checkSshConfig=` grep -rH "gerrit.sokp.co" ~/.ssh/config `
        if [ "$checkSshConfig" != "" ]; then
            gerrit=gerrit.sokp.co
            project=` git config --get remote.sokp.projectname`
            status="$1"
            git push gerrit:/$project HEAD:refs/$status/sokp-443
        else
            pspush_error
        fi
    fi
...
function taco() {
    ...
    do
        ...
        if [ $? -eq 0 ]; then
            croot
            ./vendor/sokp/bot/build_device.sh sokp_$sauce-userdebug $sauce
        else
            echo "No such item in brunch menu. Try 'breakfast'"
        fi
    done
...
function reposync() {
    case `uname -s` in
        Darwin)
            if [[ $SOKP_REPOSYNC_QUIET = true ]]; then
